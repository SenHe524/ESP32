...
#define SAMPLE_DEVICE_NAME          "MY_TEST"
...
//定义了一个名为prepare_type_env_t的结构体
typedef struct {
    uint8_t                 *prepare_buf;
    int                     prepare_len;
} prepare_type_env_t;
//定义了一个名为prepare_type_env_t的结构体变量
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  ///the app will receive this evt when the IO has Output capability and the peer device IO has Input capability.
            ...
            ESP_LOGI(EXAMPLE_TAG, "Hello!I'm here!\n");
            break;
...
void app_main(void)
{
    //定义一个esp_err_t类型的变量:ret  用于接收各种函数的返回值，对返回值进行判断，可甄别函数运行结果情况
    esp_err_t ret;

    /* Initialize NVS. */
    ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ESP_ERROR_CHECK(nvs_flash_erase());//nvs_flash_erase()：反初始化NVS分区
        ret = nvs_flash_init();
    }
    ESP_ERROR_CHECK( ret );

    //根据ESP_BT_MODE_CLASSIC_BT释放控制器内存
    ESP_ERROR_CHECK(esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT));

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    
    //初始化BT控制器以分配任务和其他资源
    ret = esp_bt_controller_init(&bt_cfg);//初始化BT控制器以分配任务和其他资源
    if (ret) {
        //如果esp_bt_controller_init失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
        ESP_LOGE(EXAMPLE_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    //使能BT控制器，不能连续两次调用此函数
    //若想更改控制模式需先调用esp_bt_controller_disable，再调用esp_bt_controller_enable
    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        //如果esp_bt_controller_enable失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
        ESP_LOGE(EXAMPLE_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    //初始化和分配蓝牙资源，此函数必须在每一个蓝牙功能函数之前
    ret = esp_bluedroid_init();
    if (ret) {
        //如果esp_bluedroid_init失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
        ESP_LOGE(EXAMPLE_TAG, "%s init bluetooth failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    //在esp_bluedroid_init之后调用，使能蓝牙
    ret = esp_bluedroid_enable();
    if (ret) {
        //如果esp_bluedroid_enable失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
        ESP_LOGE(EXAMPLE_TAG, "%s enable bluetooth failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    //调用此函数将应用程序回调注册到BTA GATTS模块。
    ret = esp_ble_gatts_register_callback(gatts_event_handler);
    if (ret){
        //如果esp_ble_gatts_register_callback失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
    ...
    if (ret){
        //如果esp_ble_gap_register_callback失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
    ...
    if (ret){
        //如果esp_ble_gatts_app_register失败，则打印失败日志信息；esp_err_to_name(ret)查询ret对应的错误信息
    ...
    //esp_ble_gap_set_security_param：设置GAP安全参数值。覆盖默认值。
