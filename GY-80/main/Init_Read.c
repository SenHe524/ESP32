...
int Init_L3G4200D(float *BUF_MARK)
{
    ...
    if (Single_Read(L3G4200_Addr, L3G4200_DEVID) != 0xD3)
        return 0;
    //数据输出速率400Hz
    if (!Single_Write(L3G4200_Addr, L3G4200_CTRL_REG1, 0xDF))
        return 0;
    if (!Single_Write(L3G4200_Addr, L3G4200_CTRL_REG2, 0x20))
        return 0;
...
int Init_ADXL345(void)
{
    ...
    if (!Single_Write(ADXL345_Addr, ADXL345_POWER_CTL, 0x08))
        return 0;
    //使能 DATA_READY 中断
    if (!Single_Write(ADXL345_Addr, ADXL345_INT_ENABLE, 0x80))
        return 0;
    return 1;
}
