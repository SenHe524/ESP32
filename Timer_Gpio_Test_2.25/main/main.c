...
// int flag_timer0_0 = 0;
// int flag_timer0_1 = 0;
int flag_gpio_4 = 0;//传感器i/o的标志位
int flag_gpio_18 = 0;//蜂鸣器i/o的标志位
...
static void timer_control(timer_test_event_info_t evt);
...
void app_main(void)
{
    ...
    while(1)
    {
        ...
        xQueueReceive(timer_Queue_t,&evt,portMAX_DELAY);
        timer_control(evt);
        Buzzer_control(evt);
        printf("\n");
        printf("/*************************/\n");
        ...
        if(evt.timer_test_info.auto_reload){
            printf("Auto_reload!\n");
        }else{
            printf("No auto_reload!\n");
        }
...
static void gpio_task_example(void* arg)
{
    ...
    for(;;) {//等待io口中断信息
        if(xQueueReceive(gpio_Queue_t, &io_num, portMAX_DELAY)) {
            ...
            //中断来后，将传感器i/o的标志位flag_gpio_4置1
            //蜂鸣器i/o的标志位flag_gpio_18置1
